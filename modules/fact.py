    ...
    with st.form("add_fact_form", clear_on_submit=True):
        ...
        if submitted:
            if not new_code or not new_name:
                ...
                try:
                    execute_modify("INSERT INTO fact (fact_code, fact_name, remark) VALUES (?, ?, ?)", 
            ...
            with st.form("edit_fact_form"):
                ...
                with col1:
                    if st.form_submit_button("更新"):
                        try:
                            execute_modify("UPDATE fact SET fact_name = ?, remark = ? WHERE fact_code = ?", 
                ...
                with col2:
                    if st.form_submit_button("刪除"):
                        try:
                            execute_modify("DELETE FROM fact WHERE fact_code = ?", (target_code,))
                            st.success("刪除成功！")
                            st.rerun()
                        except Exception as e:
                            st.error(f"刪除失敗: {e}")
